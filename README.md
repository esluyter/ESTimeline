...
~timeline = ESTimeline([ ESTrack([ ESPatternClip(0, 50, {Pbind(
  \instrument, \sin,
  \verbbus, ~verbbus,
...
)}, 99479456) ]), ESTrack([ ESSynthClip(0, 37.51, {'verb'}, {[
  verbbus: ~verbbus
...
10.do { |i|
  ...
  s.bind { syn.free };
  0.2.wait;
};}, 1234300736, cleanupFunc: {var syn;
s.bind { syn = Synth(\default) };
0.2.wait;
s.bind { syn.free };
}), ESSynthClip(20, 10, 'default', [freq: 220, amp: 0.1, pan: 0]) ]) ], 2.81, {SynthDef(\sin, { |out, freq = 100, gate = 1, amp = 0.1, preamp = 1.5, attack = 0.001, release = 0.01, pan, verbbus, verbamt, vibrato = 0.2|
  var env, sig;
...
~scrollView = ScrollView(~window, Rect(0, 40, ~window.bounds.width, ~window.bounds.height - 40)).hasHorizontalScroller_(false).hasBorder_(false);
~view = ESTimelineView(~scrollView, Rect(0, 0, ~window.bounds.width, ~window.bounds.height - 40), ~timeline, duration:60);
~makeDependant = {
