ESRoutineClipEditView : ESClipEditView {
  *new { |clip, timeline|
    ...
    funcButton = Button(editorWindow, Rect(0, 0, 100, 30)).states_([["func"], ["func", Color.white, Color.gray(0.6)]]).value_(1).action_({ funcButton.value_(1); cleanupFuncButton.value_(0); funcView.visible_(true); cleanupFuncView.visible_(false); });
    ...
    /* TODO: make this open / load whichever view is currently visible */
    Button(sidePanel, Rect(0, 410, 180, 25)).string_("Open in IDE").action_({
      Document.new("Edit Routine Clip", funcView.string).promptToSave_(false).front;
    });
